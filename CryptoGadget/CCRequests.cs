...
namespace CryptoGadget {
	...
	class CCRequest { // CryptoCompareRequest
		...
		public static Bitmap DownloadIcon(string query) {
			Global.DbgPrint("Trying..");
			WebClient client = CloudflareEvader.CreateBypassedWebClient("https://www.cryptocompare.com");
			Global.DbgPrint("Solved! We're clear to go");
	...
	/* Credit goes to https://stackoverflow.com/a/32426051 (user https://stackoverflow.com/users/5296568/maximilian-gerhardt)
	*/
		...
		public static WebClient CreateBypassedWebClient(string url) {
			...
			try {
			...
			} catch(WebException ex) //We usually get this because of a 503 service not available.
			  {
				...
				Thread.Sleep(5500); //This sleeping IS requiered or cloudflare will not give you the token!!
